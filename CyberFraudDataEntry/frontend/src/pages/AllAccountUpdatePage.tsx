...
import type { AllAccountListItem } from '../types';

const TYPE_FILTERS: ('' | 'Victim' | 'Mule')[] = ['', 'Victim', 'Mule'];
...
export function AllAccountUpdatePage() {
  ...
  const [typeFilter, setTypeFilter] = useState<'' | 'Victim' | 'Mule'>('');
      ...
      <div className="flex items-center justify-between mb-4 gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm font-medium" style={{ color: 'var(--ksp-red)' }}>
            {rows.length} record{rows.length === 1 ? '' : 's'} · {summary.victims} victim · {summary.mules} mule
          </p>
        </div>
              ...
              <tr key={r.id}
                ...
                <td className="px-3 py-2">
                  <span className="px-2 py-0.5 rounded-full text-xs font-semibold"
                    style={{
                      background: r.account_type === 'Victim' ? 'rgba(10,107,40,0.12)' : 'rgba(177,0,0,0.10)',
                      color:      r.account_type === 'Victim' ? '#0a6b28'             : '#8b1919',
                    }}>
                    {r.account_type}
                  </span>
                </td>
