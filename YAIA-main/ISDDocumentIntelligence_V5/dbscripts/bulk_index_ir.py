#!/usr/bin/env python3
"""
bulk_index_ir.py — Bulk index IR (Form-16) DOCX/PDF files into ISD Document Intelligence V5.
...
Features:
  ...
  - Supports        : .docx, .doc, .pdf files (recursively scanned)

Usage (run from the dbscripts folder):
  python bulk_index_ir.py --folder "/path/to/IR_Files" --case-id 0 --username admin --password secret
  python bulk_index_ir.py --folder "/path/to/IR_Files" --case-id 0 --workers 8
  python bulk_index_ir.py --folder "/path/to/IR_Files" --case-id 0 --reset   # clear progress, start fresh
...
DEFAULT_WORKERS    = 3
DEFAULT_COLLECTION = "IR"
# Progress DB stored in the same folder as this script
PROGRESS_DB_NAME   = os.path.join(os.path.dirname(os.path.abspath(__file__)), ".ir_bulk_progress.db")
# Refresh token 1 hour before the 24-hour window closes
TOKEN_REFRESH_SECS = 3_600
# Supported file extensions for IR documents
IR_EXTENSIONS      = {".docx", ".doc", ".pdf"}
    ...
    try:
        with open(file_path, "rb") as fh:
            resp = requests.post(
                ...
                timeout=600,  # IR docs can be large — 10 min timeout
            )
        resp.raise_for_status()
        data = resp.json()
...
def main():
    parser = argparse.ArgumentParser(
        description="Bulk index IR (Form-16) DOCX/PDF files into ISD Document Intelligence V5.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Index all IR files in a folder
  python bulk_index_ir.py --folder "/data/IR_Files" --case-id 0 --username admin --password secret

  # Use 8 parallel workers (recommended for H100)
  python bulk_index_ir.py --folder "/data/IR_Files" --workers 8

  # Clear progress and start fresh (will re-index everything)
  python bulk_index_ir.py --folder "/data/IR_Files" --reset

  # Resume after interruption (just run the same command again)
  python bulk_index_ir.py --folder "/data/IR_Files"
        """,
    )
    parser.add_argument("--folder",
                        required=True,
                        help="Folder containing IR files (.docx, .doc, .pdf) — scanned recursively")
    ...
    parser.add_argument("--progress-db",
                        ...
                        help=f"Path to SQLite progress file (default: dbscripts/.ir_bulk_progress.db)")
    ...
    # ── Collect IR files (case-insensitive, deduplicated) ─────────────────
    raw = []
    for ext in IR_EXTENSIONS:
        raw.extend(folder.rglob(f"*{ext}"))
        raw.extend(folder.rglob(f"*{ext.upper()}"))
    ...
    if not all_files:
        print(f"No IR files (.docx, .doc, .pdf) found in: {folder}")
        sys.exit(0)

    # ── Dry-run mode ───────────────────────────────────────────────────────
    if args.dry_run:
        print(f"Dry-run: found {len(all_files)} IR files in {folder}")
    ...
    print()
    print("=" * 62)
    print("  ISD Document Intelligence — Bulk IR Indexer")
    print("=" * 62)
    ...
    if n_failed:
        ...
        with open(fail_log, "w", encoding="utf-8") as fout:
            fout.write(f"# Failed IR files — {datetime.now().isoformat()}\n")
            for fp, err in fail_rows:
                fout.write(f"{fp}\t{err}\n")
