...
from config import (
    CHROMA_PATH_SMAC, EMBED_MODEL, PDF_MODEL,
    ENABLE_HYBRID_SEARCH, ENABLE_MULTI_QUERY, ENABLE_RERANKING,
    USE_LLM_PARSER_SMAC, USE_LLM_PARSER_IR,
)
from ollama_client import ollama_embed, ollama_embed_batch, ollama_chat
from structured_tables import store_smac_report, search_fields, execute_sql_query, get_table_schema_description
...
# -------------------------------------------------------------------
# Chroma persistent client — SMAC collections only
# -------------------------------------------------------------------
_client = chromadb.PersistentClient(path=CHROMA_PATH_SMAC)
...
def index_pdf(col: str, pdf_path: str, filename: str, source: str = "digital") -> Dict[str, Any]:
    ...
    elif _is_ir and USE_LLM_PARSER_IR:
        try:
...
def ask_docs(question: str, doc_ids: Optional[List[str]] = None, top_k: int = 12, collection_name: str = "SMAC", raw_question: Optional[str] = None) -> Dict[str, Any]:
    ...
    if _is_ir and not doc_ids:
        _scope_stop = {
            ...
            "interrogation", "interrogator", "interrogators", "investigating",
            "report", "ir", "all", "list", "name", "address",
            "date", "birth", "place", "number", "crime", "case",
            "family", "father", "mother", "brother", "sister", "wife", "husband",
            "weapon", "arms", "travel", "training", "organization", "organisation",
            "mobile", "phone", "bank", "account", "property", "vehicle",
        }
        words = re.findall(r"[a-zA-Z]{3,}", q_for_keywords)
        name_words = [w for w in words if w.lower() not in _scope_stop]
        print(f"[RAG:IR] name_words extracted: {name_words}")
        if name_words:
            try:
                matched_docs = find_ir_docs_by_name(name_words)
                if matched_docs:
                    doc_ids = [d["doc_id"] for d in matched_docs]
                    doc_names = [d["doc_name"] for d in matched_docs]
                    print(f"[RAG:IR] Auto-scoped to {len(doc_ids)} docs via MySQL: {doc_names[:5]}")
            except Exception as e:
