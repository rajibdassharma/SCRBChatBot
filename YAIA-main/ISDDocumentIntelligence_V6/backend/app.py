...
# App
# ------------------------------------------------------------------------------
app = FastAPI(title="ISD Document Intelligence V6")
...
async def docs_upload(
    ...
    try:
        ...
        # Route to IR or SMAC pipeline based on collection
        _is_ir_col = col == "IR" or col.startswith("IR_c")
        if _is_ir_col:
            indexed = rag_ir.index_ir(tmp_path, file.filename, source=source)
        else:
            indexed = index_document_smac(tmp_path, file.filename, collection_name=col, source=source)
...
def docs_list(
    ...
    try:
        ...
        _is_ir_col = col == "IR" or col.startswith("IR_c")
        if _is_ir_col:
            docs = rag_ir.get_indexed_doc_list()
        else:
            docs = get_indexed_doc_list_smac(collection_name=col)
        # Fallback: also include docs from the global (unscoped) collection
        if case_id and not _is_ir_col:
            try:
                global_docs = get_indexed_doc_list_smac(collection_name=collection)
                if global_docs:
...
def docs_ask(
    ...
    try:
        ...
        # Route to IR or SMAC pipeline
        _is_ir_col = col == "IR" or col.startswith("IR_c")
        if _is_ir_col:
            result = rag_ir.ask(
                question=payload.question,
                doc_ids=[payload.doc_id] if payload.doc_id else None,
                collection_name=col,
            ...
            if payload.case_id:
                try:
                    if not get_indexed_doc_list_smac(collection_name=col):
...
# ------------------------------------------------------------------------------
# QA Testing Endpoints — removed in V6
