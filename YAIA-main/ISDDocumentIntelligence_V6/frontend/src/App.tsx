...
export default function App() {
  ...
  // IR document selection state
  const [irSelectedDoc, setIrSelectedDoc] = useState<{ doc_id: string; doc_name: string } | null>(null)
  const [irMultipleDocs, setIrMultipleDocs] = useState<{ doc_id: string; doc_name: string }[]>([])
  ...
  const handleDocAsk = async () => {
    ...
    try {
      // For IR: pass stored doc_id if available
      const payload = {
        doc_id: (activeCollection === 'IR' && irSelectedDoc) ? irSelectedDoc.doc_id : null,
        question,
        history: docChat,
      ...
      const data = await apiFetch<{
        ...
        error?: string
        doc_name?: string
        used_chunks?: { doc_id?: string; doc_name?: string }[]
        multiple_docs?: { doc_id: string; doc_name: string }[]
      }>('/docs/ask', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      // Handle multiple IR documents found
      if (data?.multiple_docs && data.multiple_docs.length > 0) {
        setIrSelectedDoc(null)  // Clear current selection so dropdown shows
        setIrMultipleDocs(data.multiple_docs)
        setDocChat((prev) => [
          ...prev,
          { role: 'user', content: question },
          { role: 'assistant', content: `Multiple documents found. Please select one from the list below.` },
        ])
        setDocStatus('Multiple documents found — please select one.')
        setDocLoading(false)
        return
      }

      // Update IR document selection from response (handles name changes)
      if (activeCollection === 'IR' && data?.doc_name) {
        const chunks = data.used_chunks
        if (chunks && chunks.length > 0 && chunks[0].doc_id) {
          setIrSelectedDoc({ doc_id: chunks[0].doc_id, doc_name: data.doc_name })
        }
      }
              ...
              <div>
                ...
                <div className="upload-row">
                  <select
                    ...
                    onChange={(e) => { setActiveCollection(e.target.value as 'SMAC' | 'IR'); setIrSelectedDoc(null); setIrMultipleDocs([]); }}
                ...
                {/* IR Document Selection Banner */}
                {activeCollection === 'IR' && irSelectedDoc && (
                  <div style={{ background: '#0b2c4a', color: '#ffd400', padding: '8px 14px', borderRadius: '10px', marginBottom: '8px', fontSize: '13px', fontWeight: 600 }}>
                    Currently asking about: {irSelectedDoc.doc_name}
                  </div>
                )}

                {/* IR Multiple Documents Dropdown */}
                {activeCollection === 'IR' && irMultipleDocs.length > 0 && !irSelectedDoc && (
                  <div style={{ background: '#fff', border: '2px solid #0b2c4a', borderRadius: '10px', padding: '12px', marginBottom: '8px' }}>
                    <p style={{ fontSize: '13px', fontWeight: 600, color: '#0b2c4a', marginBottom: '8px' }}>Multiple documents found. Please select one:</p>
                    <select
                      style={{ width: '100%', padding: '8px', borderRadius: '8px', border: '2px solid #0b2c4a', fontSize: '13px' }}
                      defaultValue=""
                      onChange={(e) => {
                        const selectedId = e.target.value
                        if (!selectedId) return
                        const selectedDoc = irMultipleDocs.find(d => d.doc_id === selectedId)
                        if (selectedDoc) {
                          setIrSelectedDoc(selectedDoc)
                          setIrMultipleDocs([])
                        }
                      }}
                    >
                      <option value="">-- Select a document --</option>
                      {irMultipleDocs.map((d) => (
