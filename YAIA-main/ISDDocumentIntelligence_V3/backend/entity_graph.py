...
# ---------------------------------------------------------------------------
def _resolve_entity_id(conn: pyodbc.Connection, name: str, doc_id: str) -> Optional[int]:
...
def _insert_or_get_entity(
    ...
    cur = conn.execute(
        ...
        INSERT INTO entities (name, type, doc_id, doc_name, context)
        OUTPUT INSERTED.id
        VALUES (?, ?, ?, ?, ?)
        """,
        (name, etype, doc_id, name, etype, doc_id, doc_name, context),
    )
    row = cur.fetchone()
    if row:
...
def _insert_relationship_if_absent(
    ...
    conn.execute(
        ...
        INSERT INTO relationships (source_entity_id, target_entity_id, relationship_type, doc_id, context)
        VALUES (?, ?, ?, ?, ?)
        """,
        (src_id, tgt_id, rel_type, doc_id, src_id, tgt_id, rel_type, doc_id, context),
    )
...
def store_entities_and_relationships(
    ...
    if relationships:
        for r in relationships:
            ...
            if not src_id:
                src_id = _resolve_entity_id(conn, r["source"], doc_id)
            if not tgt_id:
                tgt_id = _resolve_entity_id(conn, r["target"], doc_id)
...
def extract_and_store_entities(
    ...
    return store_entities_and_relationships(doc_id, doc_name, unique_entities, unique_rels)
...
def get_all_entities(type_filter: Optional[str] = None) -> List[Dict[str, Any]]:
...
def clear_graph_data() -> Dict[str, Any]:
