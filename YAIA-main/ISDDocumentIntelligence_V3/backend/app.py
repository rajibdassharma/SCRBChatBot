...
# App
# ------------------------------------------------------------------------------
app = FastAPI(title="ISD Document Intelligence")
...
async def docs_upload(file: UploadFile = File(...), collection: str = Form("SMAC")):
    try:
        ...
        indexed = index_document(tmp_path, file.filename, collection_name=collection)
...
def docs_extract_entities():
    ...
    def _run_all():
        ...
        for i, job in enumerate(jobs):
            try:
                extract_and_store_entities(job["doc_id"], job["doc_name"], job["chunks"])
...
def graph_extract_all(collection: str = "SMAC"):
    ...
    try:
        ...
        doc_groups = get_all_doc_chunks(collection_name=collection, field_filter=field_filter)
        if not doc_groups:
        ...
        def _run_all():
            ...
            for i, dg in enumerate(doc_groups):
                try:
                    extract_and_store_entities(dg["doc_id"], dg["doc_name"], dg["chunks"])
                except Exception as ex:
...
# ------------------------------------------------------------------------------
@app.get("/docs/list")
def docs_list(collection: str = "SMAC"):
    try:
        docs = get_indexed_doc_list(collection_name=collection)
...
# ------------------------------------------------------------------------------
@app.post("/docs/clear")
def clear_docs(collection: str = "SMAC"):
    out = clear_all_documents(collection_name=collection)
    if out.get("ok"):
        try:
            clear_graph_data()
        except Exception as e:
...
def docs_agent(payload: DocAgentQuestion):
    try:
        ...
        result = ask_docs_agent(question=question, doc_ids=payload.doc_ids, top_k=6, collection_name=payload.collection)
        return {"ok": True, **result}
...
def graph_entities(type: Optional[str] = None):
    try:
        entities = get_all_entities(type_filter=type)
...
def timeline_extract_all(collection: str = "SMAC"):
    ...
    try:
        ...
        already_extracted = get_extracted_doc_ids()
        ...
        def _run_all():
            ...
            for i, dg in enumerate(new_docs):
                try:
                    extract_and_store_activities(dg["doc_id"], dg["doc_name"], dg["chunks"])
                except Exception as ex:
...
def timeline_data(group: Optional[str] = None, status: Optional[str] = None, search: Optional[str] = None):
    try:
        activities = get_timeline_data(group_filter=group, status_filter=status, search=search)
...
def timeline_breadcrumb(tms_id: str):
    try:
        trail = get_breadcrumb_trail(tms_id)
...
def locations_extract_all(collection: str = "IR"):
    ...
    try:
        ...
        doc_groups = get_all_doc_chunks(collection_name=collection)
        if not doc_groups:
            return {"ok": True, "message": "No documents in ChromaDB to extract from.", "total": 0}

        # Incremental: skip docs already extracted
        already_extracted = get_extracted_doc_ids_for_locations()
        new_docs = [dg for dg in doc_groups if dg["doc_id"] not in already_extracted]
        ...
        def _run_all():
            ...
            for i, dg in enumerate(new_docs):
                try:
                    extract_and_store_locations(dg["doc_id"], dg["doc_name"], dg["chunks"])
                except Exception as ex:
