...
def parse_ir_docx(docx_path: str) -> Tuple[List[Dict[str, str]], str, str]:
    """
    Parse an IR Form-16 DOCX file.

    Returns:
        (fields, accused_name, statement_text)
        - fields: List of {serial_no, field_key, field_value}
        - accused_name: Name extracted from the document
        - statement_text: Free-form narrative text (paragraphs outside/after tables)
    """
    try:
    ...
    # Extract paragraph text (statement/narrative — text outside tables)
    statement_parts = []
    for para in doc.paragraphs:
        ...
        if t and len(t) > 10:
            ...
            t_lower = t.lower()
            if t_lower in ("section 1", "section 2", "section 3", "section 4", "section 5"):
                continue
            if re.match(r"^\d+\s*\|\s*p\s*a\s*g\s*e$", t_lower):
                continue
            statement_parts.append(t)

    statement_text = "\n\n".join(statement_parts)

    print(f"[IR-Parser] Extracted {len(fields)} fields from {len(doc.tables)} tables, "
          f"{len(statement_parts)} paragraphs ({len(statement_text)} chars) in {os.path.basename(docx_path)}")
    return fields, accused_name, statement_text


def parse_ir_pdf(pdf_path: str) -> Tuple[List[Dict[str, str]], str, str]:
    """
    Parse an IR PDF file. Extracts text with pypdf, then parses
    Returns: (fields, accused_name, statement_text)
    ...
    # For PDFs, the full extracted text that wasn't parsed as fields is the statement
    all_text = []
    for page in reader.pages:
        try:
            text = (page.extract_text() or "").strip()
...
def parse_ir_document(file_path: str, filename: str) -> Tuple[List[Dict[str, str]], str, str]:
    """
    Parse any IR document (DOCX, DOC, or PDF).
