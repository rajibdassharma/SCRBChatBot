<#
.SYNOPSIS
    Run automated integration tests for ISD Document Intelligence V4.

.DESCRIPTION
    Prerequisites:
      1. V4 backend must be running:   .\start_v4_backend.ps1
      2. MSSQL (ISDIntelligenceV4) must be accessible
      3. Anaconda Python must be in PATH or use full path below
...
$BACKEND  = "YAIA-main\ISDDocumentIntelligence_V4\backend"
...
try {
    ...
    Write-Host "Please start it first: .\start_v4_backend.ps1" -ForegroundColor Yellow
    exit 1
}
