...
import { searchCaseByFir } from '../lib/api/cases';
...
export function CaseUpdatePage() {
  ...
  const handleSearch = async () => {
    if (!firNo.trim()) return;
    setSearching(true);
    setError('');
    try {
      const result = await searchCaseByFir(firNo.trim());
      if (result && result.id) {
        navigate(`/cases/${result.id}`);
      } else {
        setError('No case found with this FIR number');
      }
    } catch (err) {
  ...
  return (
    <div>
      ...
      <h1 className="text-[22px] font-bold mb-1" style={{ color: 'var(--ksp-navy)' }}>Update Case</h1>
      <p className="text-sm font-medium mb-6" style={{ color: 'var(--ksp-red)' }}>Search for a case by FIR number to edit it</p>
      ...
      <div className="max-w-xl">
        <div className="rounded-2xl p-6" style={{ background: '#fff', border: '1px solid rgba(0,0,0,0.06)', boxShadow: '0 6px 16px rgba(0,0,0,0.08)' }}>
          <label className="block text-xs font-semibold mb-2" style={{ color: 'var(--ksp-navy)' }}>Enter FIR No</label>
