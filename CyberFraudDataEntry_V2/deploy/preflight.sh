...
#
# Usage:
#   cd /opt/scrb && bash CyberFraudDataEntry_V2/deploy/preflight.sh
...
echo "=== CyberFraud V2 pre-deploy checklist ==="
echo "  SOURCE  $SOURCE"
echo "  REPO    $REPO_ROOT"
...
check_path "nginx site config"          /etc/nginx/sites-available/cyberfraud_v2
...
if systemctl list-unit-files --no-pager 2>/dev/null | grep -q '^cyberfraud-v2-backend\.service'; then
    ok   "cyberfraud-v2-backend service unit installed"
else
    nope "cyberfraud-v2-backend service unit installed" "(no systemd unit file)"
fi

# OS user the runtime files are chowned to
if id cyberfraud_v2 >/dev/null 2>&1; then ok   "cyberfraud_v2 OS user exists"
else                                      nope "cyberfraud_v2 OS user exists"; fi
...
if [ $fail -eq 0 ]; then
    ...
    echo "    bash CyberFraudDataEntry_V2/deploy/redeploy-vapt-fixes.sh"
    exit 0
else
