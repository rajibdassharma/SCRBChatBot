# Project: Cyber Fraud Data Entry — **V2 (in development)**

> **THIS IS V2.** V1 lives in the sibling `CyberFraudDataEntry/` folder
> and is in production on the server. Do not edit V1 from this folder.
> Do not point V2 work at V1's database, port, runtime path, or systemd
> service.
>
> | Aspect | V1 (production) | V2 (this folder, in dev) |
> |---|---|---|
> | Folder | `CyberFraudDataEntry/` | `CyberFraudDataEntry_V2/` |
> | DB name | `cyber_fraud_dsr` | `cyber_fraud_dsr_v2` |
> | Backend port | 8000 | 8001 |
> | Frontend dev port | 5175 | 5176 |
> | Server runtime path | `/opt/cyberfraud/` | `/opt/cyberfraud_v2/` |
> | systemd service | `cyberfraud-backend` | `cyberfraud-v2-backend` |
> | OS user (server) | `cyberfraud` | `cyberfraud_v2` |
> | nginx HTTPS port | 443 | 8443 |
> | nginx site name | `cyberfraud` | `cyberfraud_v2` |
> | Server log dir | `/var/log/nginx/cyberfraud_*` | `/var/log/nginx/cyberfraud_v2_*` |
>
> V2 adds: 3 unit_users per PS, central_admin role, senior_officer role
> (DGP/ADGP/SP), audit_log table, view-only mode for senior officers,
> user-management UI for the central admin. See PLAN.md.
...
- **Production server (V2 deploy slot)**: Ubuntu 24.04, Nginx + Gunicorn + Uvicorn on port 8001
- **Frontend**: built `dist/` served by Nginx on port 8443 (HTTPS with self-signed cert; V1 keeps 443)
- **Server paths**: code at `/opt/cyberfraud_v2/`, systemd service `cyberfraud-v2-backend`
- **MySQL on server**: same instance, separate database `cyber_fraud_dsr_v2`
